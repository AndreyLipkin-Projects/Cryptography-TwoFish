...
public class TwoFish {
	...
	//plaintText and key are expected to be of length 4 = 16 bytes = 128 bit
	public static int[] encrypt(int[] plainText, int[] key, boolean debug) {
		...
		// Swapping then output whitening
		whitened = new int[] { whitened[2], whitened[3], whitened[0], whitened[1] };
	...
	public static int[] decrypt(int[] cypheredText, int[] key, boolean debug) {
		if (debug) {
			...
			System.out.println(Utils.writeInput(cypheredText));
		}
		// get whitening keys
		final int[] roundKey01 = roundKeys(key, 0);
		final int[] roundKey23 = roundKeys(key, 1);
	...
	//ecryption round
	private static int[] encryptionRound(int[] input, int[] key, int round) {
	...
	//decryption round
	private static int[] decryptionRound(int[] input, int[] key, int round) {
		final int[] s = getS(key);
	...
	//pht function
	private static int[] pht(int a, int b) {
		int a1 = a + b;
		int b1 = (a + 2 * b);
		return new int[] { a1, b1 };
	}
	//H function
	private static int h(int input, int l0, int l1) {
	...
	//q0 permutation
	private static byte q0(byte input) {
	...
	//q1 permutation
	private static byte q1(byte input) {
		byte a0 = (byte) ((input >> 4) & 0xF);
		byte b0 = (byte) (input & 0xF);
		byte a1 = (byte) (a0 ^ b0);
		byte b1 = (byte) (a0 ^ ((b0 & 1) << 3 | b0 >> 1) ^ ((8 * a0) & 0xF));
	...
	//get S sub-keys
	private static int[] getS(int[] key) {
		final int m0 = key[0];
		final int m1 = key[1];
		final int m2 = key[2];
		final int m3 = key[3];
	...
	private static int RS(int X, int Y) {
		...
		//multiplications are carried over the finite Galoi field GF(256) 
	...
	//get round keys (AKA K sub-keys)
	private static int[] roundKeys(int[] key, int round) {
		final int m0 = key[0];
		final int m1 = key[1];
		final int m2 = key[2];
		final int m3 = key[3];
		//define M_even and M_odd
